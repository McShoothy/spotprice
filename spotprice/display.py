...
from .config import (
    BLACK, WHITE, GREEN, LIGHT_GREEN, YELLOW, RED, BLUE, ACCENT, GRAY,
    PRICE_LOW, PRICE_MED, PRICE_HIGH, PRICE_MAX, DISPLAY_BRIGHTNESS
)
...
def show_graph(prices, now_index=0):
    ...
    min_y = int(graph_bottom - ((min_price - min_price_display) / price_range) * graph_height)
    min_line_pal = displayio.Palette(1)
    min_line_pal[0] = 0x004400
    for x in range(graph_left, graph_right, 4):
        dash = vectorio.Rectangle(pixel_shader=min_line_pal, width=2, height=1, x=x, y=min_y)
        graph_group.append(dash)
    
    # Draw horizontal line at max price
    max_y = int(graph_bottom - ((max_price - min_price_display) / price_range) * graph_height)
    max_line_pal = displayio.Palette(1)
    max_line_pal[0] = 0x440000
    for x in range(graph_left, graph_right, 4):
        dash = vectorio.Rectangle(pixel_shader=max_line_pal, width=2, height=1, x=x, y=max_y)
        graph_group.append(dash)
    ...
    for i, price in enumerate(prices):
        ...
        y = int(graph_bottom - ((price - min_price_display) / price_range) * graph_height)
    ...
    for i in range(len(points) - 1):
        ...
        for x in range(x1, x2 + 1):
            ...
            if graph_bottom > y:
                ...
                line = vectorio.Rectangle(pixel_shader=fill_pal, width=1, height=graph_bottom - y, x=x, y=y)
    ...
    for i in range(len(points) - 1):
        ...
        for step in range(steps + 1):
            ...
            x = int(x1 + t * dx)
            y = int(y1 + t * dy)
            dot = vectorio.Rectangle(pixel_shader=line_pal, width=2, height=2, x=x, y=y - 1)
    ...
    if points and now_index < len(points):
        ...
        marker = vectorio.Rectangle(pixel_shader=marker_pal, width=4, height=4, x=now_x - 2, y=now_y - 2)
        ...
        for y_pos in range(graph_top, graph_bottom, 3):
            dot = vectorio.Rectangle(pixel_shader=now_line_pal, width=1, height=2, x=now_x, y=y_pos)
            graph_group.append(dot)
    ...
    for h in range(-past_hours, total_hours - past_hours, hour_interval):
        ...
        if 0 <= slot_index < len(prices):
            ...
            tick_pal[0] = GRAY
            tick = vectorio.Rectangle(pixel_shader=tick_pal, width=1, height=4, x=x_pos, y=graph_bottom)
